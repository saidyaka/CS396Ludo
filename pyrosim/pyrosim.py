...
def Send_Link(name,pos, objectType, size, sensor):
    ...
    if filetype == SDF_FILETYPE:
        ...
        link = LINK_URDF(name,pos,size, objectType, sensor)
...
def Send_Cube(name="default",pos=[0,0,0],size=[1,1,1], sensor=False):

    Send_Link(name,pos,"box", size, sensor)

def Send_Sphere(name="default",pos=[0,0,0],radius=0.5 , sensor=False):

    Send_Link(name, pos, "sphere", size=[radius], sensor = sensor)

def Send_Cylinder(name="default",pos=[0,0,0],length=0.5, radius=1 , sensor=False):

    Send_Link(name,pos,"cylinder", size=[length, radius], sensor = sensor)
